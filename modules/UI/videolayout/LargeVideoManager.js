...
export default class LargeVideoManager {
    ...
    destroy() {
        ...
        if (getSourceNameSignalingFeatureFlag(APP.store.getState())) {
            ...
            if (this.videoTrack && !this.videoTrack.local) {
                this.videoTrack.jitsiTrack.off(
                    JitsiTrackEvents.TRACK_STREAMING_STATUS_CHANGED,
                    this.handleTrackStreamingStatusChanged
                );
                APP.store.dispatch(
                    trackStreamingStatusChanged(
                        this.videoTrack.jitsiTrack,
        ...
        preUpdate
            .then(() => {
                ...
                if (getSourceNameSignalingFeatureFlag(state)) {
                    ...
                    // use a custom hook to update a local track streaming status.
                    if (
                        this.videoTrack?.jitsiTrack?.getSourceName() !==
                        videoTrack?.jitsiTrack?.getSourceName()
                    ) {
                        if (this.videoTrack && !this.videoTrack.local) {
                            this.videoTrack.jitsiTrack.off(
                                JitsiTrackEvents.TRACK_STREAMING_STATUS_CHANGED,
                    ...
                    this.videoTrack?.jitsiTrack?.getVideoType() ===
                        VIDEO_TYPE.DESKTOP &&
                        logger.debug(
                            `Remote track ${videoTrack?.jitsiTrack}, isVideoMuted=${isVideoMuted},` +
                ...
                if (showAvatar) {
                    ...
                    if (
                        ...
                        const tracks = state["features/base/tracks"];
                        const track = getVideoTrackByParticipant(
                            tracks,
                            participant
                        );
                ...
                if (getSourceNameSignalingFeatureFlag(state)) {
                    ...
                    messageKey = isTrackStreamingStatusInactive(videoTrack)
                        ? "connection.LOW_BANDWIDTH"
                        : null;
                } else {
                    messageKey = isParticipantConnectionStatusInactive(
                        participant
                    )
                        ? "connection.LOW_BANDWIDTH"
                        : null;
    ...
    updateContainerSize(width, height) {
        ...
        if (
            resizableFilmstrip &&
            visible &&
            filmstripWidth.current >= FILMSTRIP_BREAKPOINT
        ) {
